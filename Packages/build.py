...
project_directories = ["advanced-vfr-map\html_ui", "advanced-vfr-map\InGamePanels"]
...
for project_directory in project_directories:
    for directory_path, directory_names, file_names in os.walk(project_directory):
        for file_name in file_names:
            ...
            content_entry["path"] =  content_entry["path"].replace("advanced-vfr-map/","")
...
layout_file = open("advanced-vfr-map/layout.json", "w")
...
manifest_file = open("advanced-vfr-map/manifest.json", "r")
manifest_entries = json.load(manifest_file)
manifest_entries["dependencies"]= dependencies["dependencies"]
manifest_entries["total_package_size"] = str(total_package_size).zfill(20)
manifest_file = open("advanced-vfr-map/manifest.json", "w")
